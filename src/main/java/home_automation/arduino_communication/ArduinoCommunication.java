...
public class ArduinoCommunication {
	...
	private ArduinoCommunication(String serverIP, int serverPort)
		...
		logger.info(Markers.CONSTRUCTOR, "Connecting to Arduino with:" + serverIP
				+ " on port:" + serverPort);
		clientSocket = new Socket(serverIP, serverPort);
		logger.info(Markers.CONSTRUCTOR, "Connected to Arduino with:" + serverIP
				+ " on port:" + serverPort);
	...
	public synchronized void pwm(int port, int percentage) throws IOException, PercentageOutOfRange {
		...
		logger.info(Markers.PWM,"Setting port "+port+" with pwm value of {}", pwmValue);
