...
//	MainBedroom mb = new MainBedroom(AC);
		...
		// serverIP =
		// settings.getArduino().get(1).getAddress().getHost().getIp();
		// ArduinoCommunication a = ArduinoCommunication.getInstance(serverIP,
		// serverPort);
		//
		...
		// LightApplication light = k.addLight(new
		// LightApplication(LightType.PWM,a,k, "ceiling light", 3));
		// MotorApplication motor = k.addMotor(new MotorApplication(a, k,
		// "blindes", lmotor));
		// Sensor sensor = k.addSensor(new
		// Sensor(SensorType.ANGLE_SENSOR,a,k,"sensor",6));
		//
		//
